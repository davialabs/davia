...
export function getBaseDestinationPath(
  ...
  if (isUpdate) {
    return path.join(daviaPath, "proposed", projectId);
  }
  return path.join(daviaPath, "assets", projectId);
}

/**
 * Get the assets path (for reading existing files)
 */
export function getAssetsPath(daviaPath: string, projectId: string): string {
  return path.join(daviaPath, "assets", projectId);
}
 ...
 * @param context - Runtime context with daviaPath, projectId, and isUpdate
